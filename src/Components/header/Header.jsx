    ...
    return (
        <header className="relative bg-gradient-to-br from-indigo-500 to-indigo-900 flex items-end w-full h-[25vh] md:h-[45vh] md:mt-4 rounded-lg overflow-hidden">
            <div className="w-3/4 flex flex-col gap-4 p-4 text-indigo-200 z-50">
                <h1 className="text-2xl md:text-4xl font-bold">{headerInfo[banerIndex]?.title}</h1>
                <p className="text-sm md:text-xl font-medium">{headerInfo[banerIndex]?.describtion}</p>
                <Link
                    className="bg-blue-500 w-fit p-2 rounded-xl text-white cursor-pointer"
                    href={headerInfo[banerIndex]?.link}
                >{headerInfo[banerIndex]?.btnText}</Link>
            </div>
            <Image
                src={headerInfo[banerIndex]?.img}
                className='absolute -left-10 -bottom-5 md:left-20 md:-bottom-10 md:w-1/3'
                alt='baner image'
                width={300}
                height={300}
            />
